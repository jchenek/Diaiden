...
#PATH input
my_path <- commandArgs(trailingOnly = TRUE)
...
diazo_cp1 <- diazo_cp1 %>%
  mutate(com <- paste("cp ",my_path,"/reformated/",diazo_nifh_col,"* ./diazo_nifh_genomes",sep = ""))
...
diazo_cp2 <- diazo_cp2 %>%
  mutate(com <- paste("cp ",my_path,"/reformated/",diazo_non_nifh_col,"* ./diazo_nifs_genomes",sep = ""))
...
diazo_cp3 <- diazo_cp3 %>%
  mutate(com <- paste("cp ",my_path,"/reformated/",diazo_col,"* ./diazo_nifh_nifs_genomes",sep = ""))
